...
class M_report extends CI_Model {
	...
	public function cari_harian($date =null,$date2=null)
	{
        ...
        $this->db->join('detail_pengiriman', 'detail_pengiriman.id_kirim = data_pengiriman.id_kirim', 'right');
	...
	public function cari_pengiriman($date =null,$date2=null)
	{
		$this->db->select('data_selesai_kirim.*', FALSE);
        $this->db->select('data_pengiriman.*', FALSE);
        $this->db->from('data_selesai_kirim');
        $this->db->join('data_pengiriman', 'data_pengiriman.id_kirim = data_selesai_kirim.id_kirim', 'right');
		$this->db->where('data_selesai_kirim.tgl_kirim BETWEEN "'.date($date).'"AND"'.date($date2).'"');
        $query_result = $this->db->get();
		return $data = $query_result->result();
	}
	public function cari_pengiriman_pertanggal($date =null,$date2=null)
	{
		$this->db->select('data_selesai_kirim.*', FALSE);
        $this->db->select('data_pengiriman.*', FALSE);
        $this->db->from('data_selesai_kirim');
        $this->db->join('data_pengiriman', 'data_pengiriman.id_kirim = data_selesai_kirim.id_kirim', 'right');
		$this->db->where('data_selesai_kirim.tgl_setor BETWEEN "'.date($date).'"AND"'.date($date2).'"');
        $query_result = $this->db->get();
		return $data = $query_result->result();
	}
