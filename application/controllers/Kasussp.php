...
class Kasussp extends AUTH_Controller {
	...
	public function index() {
		...
		$data['datakasussp'] 	= $this->M_kasussp->select_all();

		$data['page'] 		= "kasussp";
		$data['judul'] 		= "Kasus Pengemudi";
		$data['deskripsi'] 	= "Manage Data Kasus Pengemudi";

		$data['modal_tambah_kota'] = show_my_modal('modals/modal_tambah_kota', 'tambah-kota', $data);

		$this->template->views('kasussp/home', $data);
	}

	public function tampil() {
		$data['datakasussp'] = $this->M_kasussp->select_all();
		$this->load->view('kasussp/list_data', $data);
	}
	...
	public function delete() {
		$id_lk = $_POST['id_lk'];
		$result = $this->M_kasussp->delete($id_lk);
		
		if ($result > 0) {
			echo show_succ_msg('Data Kasus Pengemudi Berhasil dihapus', '20px');
		} else {
			echo show_err_msg('Data Kasus Pengemudi Gagal dihapus', '20px');
		}
	}
