...
class Transaksi extends AUTH_Controller {
	...
	public function ajax_konsumen()
	{
		...
		foreach ($list as $dataL) {
			...
			$row = array();
			$row[] = $no;
			$row[] = "<button type='button' class='btn btn-sm btn-success' onClick=selectKonsumen('".$dataL->id_konsumen."','".$dataL->nama."')> Pilih</button>";
			//<a href=javascript:selectKonsumen('".$dataL->id_konsumen."','".$dataL->nama."')>test</a>
			$row[] = $dataL->nama;
			$row[] = $dataL->no_telp;
			$row[] = $dataL->alamat;
			$data[] = $row;
